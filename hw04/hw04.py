...
def shuffle(s):
    ...
    "*** YOUR CODE HERE ***"
    left_half_len = len(s) // 2;
    right_half_len = len(s) - left_half_len
    a = s[:left_half_len]
    b = s[-right_half_len:]
    ans = []
...
def balanced(m):
    ...
    if is_planet(m):
        return True
    torque = total_mass(end(left(m))) * length(left(m)) == total_mass(end(right(m))) * length(right(m))

    return torque and balanced(end(left(m))) and balanced(end(right(m)))
...
def max_path_sum(t):
    ...
    cur_sum = label(t)
    for i in branches(t):
        cur_sum = max(cur_sum, label(t) + max_path_sum(i))
    return cur_sum
