...
def calc_eval(exp):
    ...
    if isinstance(exp, Link):
        ...
        if operator == 'and': # and expressions
            ...
            return calc_apply(calc_eval(operator), evaluated_operands) # UPDATE THIS FOR Q2, what is type(operator)?
...
def floor_div(args):
    ...
    """
    "*** YOUR CODE HERE ***"
    if args is Link.empty:
        return 1
    
    top = calc_eval(args.first)
    copy = args.rest
    while copy is not Link.empty:
        top = top // calc_eval(copy.first)
        copy = copy.rest
    return top
