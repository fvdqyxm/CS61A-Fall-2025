...
def interleaved_sum(n, f_odd, f_even):
    ...
    def odd_partial(k, n):
        if(k == n):
            return f_odd(n)
        return f_odd(k) + even_partial(k + 1, n)
    def even_partial(k, n):
        if(k == n):
            return f_even(n)
        return f_even(k) + odd_partial(k + 1, n)
    return odd_partial(1, n)
...
def count_dollars(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def get(total, cur_denom):
        if total == 0:
            return 1
        if total < 0 or cur_denom == None:
            return 0
        return get(total - cur_denom, cur_denom) + get(total,  next_smaller_dollar(cur_denom))
