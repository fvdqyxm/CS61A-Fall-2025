...
class Ant(Insect):
    ...
    def add_to(self, place: Place):
        if place.ant is None:
            ...
            cur = place.ant
            new = self
            if cur.can_contain(self):
                cur.store_ant(self)
                self.place = place
            elif self.can_contain(cur):
                self.store_ant(cur)
                place.ant = self
                self.place = place
            else:
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if getattr(self, 'doubled', False) == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        ...
        i = self.place
        cur = 0
        while i != None and i.is_hive == False:
            if self.lower_bound <= cur and cur <= self.upper_bound:
                if i.bees:
                    return random_bee(i.bees) # REPLACE THIS LINE
            i = i.entrance
            cur += 1
        # END Problem 3 and 4
    ...
    def action(self, gamestate: GameState):
        if self.cooldown > 0:
            ...
            if self.place and self.place.bees:
                which_bee = random_bee(self.place.bees)
                if which_bee is not None:
                    which_bee.reduce_health(which_bee.health);
    ...
    def action(self, gamestate: GameState):
        ...
        while cur != None:
            ...
            if i:
                i.double()
                # self.double(i)
                if getattr(i, 'ant_contained', None):
                    # if i.ant_contained:
                        # self.double(i.ant_contained)
