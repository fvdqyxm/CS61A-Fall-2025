...
def about(keywords: list[str]):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def contains(paragraphs : list [str]):
        for a in split(paragraphs):
            for b in keywords:
                # print("testing", a, b)
                if lower(remove_punctuation(a)) == remove_punctuation(b):
                    return True;
        return False;
    return contains
    # END PROBLEM 2
...
def accuracy(typed: str, source: str) -> float:
    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        ...
        for i in range(total):
            if typed_words[i] == source_words[i]:
                got += 1
        return got / len(typed_words) * 100
...
def autocorrect(typed_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    for i in range(len(word_list)):
        ...
        # print("typed word: ", typed_word, " word: ", word_list[i], " difference: ", dif, " limit: ", limit)
        if dif < cur_dif:
            cur_dif = dif
            good_word = word_list[i]
    if cur_dif > limit:
        return typed_word
    return good_word
    # END PROBLEM 5
...
def furry_fixes(typed: str, source: str, limit: int) -> int:
    ...
    def dif(a, b, limit):
        ...
        if a[0] == b[0]:
            return dif(a[1:], b[1:], limit)
        else:
            return 1 + dif(a[1:], b[1:], limit - 1); 
    return dif(typed, source, limit);
    # END PROBLEM 6
...
def report_progress(typed: list[str], source: list[str], user_id: int, upload) -> float:
    ...
    for i in range(min(a, b)):
        if typed[i] == source[i]:
            ...
            break
    upload({'id' :  user_id, 'progress' : good_words / b})
    return good_words / b
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    for i in range(len(player_indices)):
        ans.append([])
    # print(len(ans) == len(player_indices))
    for word_index in word_indices:
        cur_time = 10**30
        cur_index = -1
        for player in player_indices:
            val = get_time(times, player, word_index)
            # if get_time(times, player, word_index) <
            if val < cur_time:
                cur_time = val
                cur_index = player
            # print(word_index, player)
        ans[cur_index].append(words[word_index])
    return ans;
    # END PROBLEM 10
