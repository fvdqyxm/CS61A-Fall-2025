def divide(quotients: list[int], divisors: list[int]) -> dict[int, list[int]]:
    ...
    """
    return {a : [x for x in divisors if x % a == 0] for a in quotients}
...
def buy(fruits_to_buy: list[str], prices: dict[str, int], total_amount: int) -> None:
    ...
    def add(fruits: list[str], amount: int, cart: str) -> None:
        if fruits == [] and amount == 0:
            ...
            fruit = fruits[0]
            price = prices[fruit]
            can_buy = amount // price + 1
            if amount > 1:
                rem = fruits[1:]
            for k in range(1, can_buy):
                # Hint: The display function will help you add fruit to the cart.
                add(rem, amount - k * price, cart + display(fruit, k))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    dist_one = distance(make_city('temp', lat, lon), city_a)
    dist_two = distance(make_city('temp', lat, lon), city_b)
